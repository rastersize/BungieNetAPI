//
// Generated by SwagGen
// https://github.com/yonaskolb/SwagGen
//

import Foundation

/** The base item component, filled with properties that are generally useful to know in any item request or that don't feel worthwhile to put in their own component. */
public struct DestinyEntitiesItemsDestinyItemComponent: APIModel {

    /** If the item is bound to a location, it will be specified in this enum. */
    public var bindStatus: Int?

    /** The hash identifier for the specific inventory bucket in which the item is located. */
    public var bucketHash: Int?

    /** If the item can expire, this is the date at which it will/did expire. */
    public var expirationDate: DateTime?

    /** If this is true, the object is actually a "wrapper" of the object it's representing. This means that it's not the actual item itself, but rather an item that must be "opened" in game before you have and can use the item.
 Wrappers are an evolution of "bundles", which give an easy way to let you preview the contents of what you purchased while still letting you get a refund before you "open" it. */
    public var isWrapper: Bool?

    /** The identifier for the item's definition, which is where most of the useful static information for the item can be found. */
    public var itemHash: Int?

    /** If the item is instanced, it will have an instance ID. Lack of an instance ID implies that the item has no distinct local qualities aside from stack size. */
    public var itemInstanceId: Int?

    /** An easy reference for where the item is located. Redundant if you got the item from an Inventory, but useful when making detail calls on specific items. */
    public var location: Int?

    /** If the item can be locked, this will indicate that state. */
    public var lockable: Bool?

    /** If populated, this is the hash of the item whose icon (and other secondary styles, but *not* the human readable strings) should override whatever icons/styles are on the item being sold.
If you don't do this, certain items whose styles are being overridden by socketed items - such as the "Recycle Shader" item - would show whatever their default icon/style is, and it wouldn't be pretty or look accurate. */
    public var overrideStyleItemHash: Int?

    /** The quantity of the item in this stack. Note that Instanced items cannot stack. If an instanced item, this value will always be 1 (as the stack has exactly one item in it) */
    public var quantity: Int?

    /** A flags enumeration indicating the transient/custom states of the item that affect how it is rendered: whether it's tracked or locked for example, or whether it has a masterwork plug inserted. */
    public var state: Int?

    /** If this is populated, it is a list of indexes into DestinyInventoryItemDefinition.tooltipNotifications for any special tooltip messages that need to be shown for this item. */
    public var tooltipNotificationIndexes: [Int]?

    /** If there is a known error state that would cause this item to not be transferable, this Flags enum will indicate all of those error states. Otherwise, it will be 0 (CanTransfer). */
    public var transferStatus: Int?

    public init(bindStatus: Int? = nil, bucketHash: Int? = nil, expirationDate: DateTime? = nil, isWrapper: Bool? = nil, itemHash: Int? = nil, itemInstanceId: Int? = nil, location: Int? = nil, lockable: Bool? = nil, overrideStyleItemHash: Int? = nil, quantity: Int? = nil, state: Int? = nil, tooltipNotificationIndexes: [Int]? = nil, transferStatus: Int? = nil) {
        self.bindStatus = bindStatus
        self.bucketHash = bucketHash
        self.expirationDate = expirationDate
        self.isWrapper = isWrapper
        self.itemHash = itemHash
        self.itemInstanceId = itemInstanceId
        self.location = location
        self.lockable = lockable
        self.overrideStyleItemHash = overrideStyleItemHash
        self.quantity = quantity
        self.state = state
        self.tooltipNotificationIndexes = tooltipNotificationIndexes
        self.transferStatus = transferStatus
    }

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: StringCodingKey.self)

        bindStatus = try container.decodeIfPresent("bindStatus")
        bucketHash = try container.decodeIfPresent("bucketHash")
        expirationDate = try container.decodeIfPresent("expirationDate")
        isWrapper = try container.decodeIfPresent("isWrapper")
        itemHash = try container.decodeIfPresent("itemHash")
        itemInstanceId = try container.decodeIfPresent("itemInstanceId")
        location = try container.decodeIfPresent("location")
        lockable = try container.decodeIfPresent("lockable")
        overrideStyleItemHash = try container.decodeIfPresent("overrideStyleItemHash")
        quantity = try container.decodeIfPresent("quantity")
        state = try container.decodeIfPresent("state")
        tooltipNotificationIndexes = try container.decodeArrayIfPresent("tooltipNotificationIndexes")
        transferStatus = try container.decodeIfPresent("transferStatus")
    }

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: StringCodingKey.self)

        try container.encodeIfPresent(bindStatus, forKey: "bindStatus")
        try container.encodeIfPresent(bucketHash, forKey: "bucketHash")
        try container.encodeIfPresent(expirationDate, forKey: "expirationDate")
        try container.encodeIfPresent(isWrapper, forKey: "isWrapper")
        try container.encodeIfPresent(itemHash, forKey: "itemHash")
        try container.encodeIfPresent(itemInstanceId, forKey: "itemInstanceId")
        try container.encodeIfPresent(location, forKey: "location")
        try container.encodeIfPresent(lockable, forKey: "lockable")
        try container.encodeIfPresent(overrideStyleItemHash, forKey: "overrideStyleItemHash")
        try container.encodeIfPresent(quantity, forKey: "quantity")
        try container.encodeIfPresent(state, forKey: "state")
        try container.encodeIfPresent(tooltipNotificationIndexes, forKey: "tooltipNotificationIndexes")
        try container.encodeIfPresent(transferStatus, forKey: "transferStatus")
    }

    public static func == (lhs: DestinyEntitiesItemsDestinyItemComponent, rhs: DestinyEntitiesItemsDestinyItemComponent) -> Bool {
        guard lhs.bindStatus == rhs.bindStatus else { return false }
        guard lhs.bucketHash == rhs.bucketHash else { return false }
        guard lhs.expirationDate == rhs.expirationDate else { return false }
        guard lhs.isWrapper == rhs.isWrapper else { return false }
        guard lhs.itemHash == rhs.itemHash else { return false }
        guard lhs.itemInstanceId == rhs.itemInstanceId else { return false }
        guard lhs.location == rhs.location else { return false }
        guard lhs.lockable == rhs.lockable else { return false }
        guard lhs.overrideStyleItemHash == rhs.overrideStyleItemHash else { return false }
        guard lhs.quantity == rhs.quantity else { return false }
        guard lhs.state == rhs.state else { return false }
        guard lhs.tooltipNotificationIndexes == rhs.tooltipNotificationIndexes else { return false }
        guard lhs.transferStatus == rhs.transferStatus else { return false }
        return true
    }
}
